...
Claude was pretty helpful for edge-case test posts and explaining concepts, but there are some errors when I asked for the "right" threshold for the `netural` and `mixed` label. To guide a student who is stuck without giving the answer, I would ask them to print the token list for a failing case and describe out loud what they expect each token to contribute to the score. That can help narrow the bug to a specific step without revealing the fix.
